...
Shader "NMP/Wood" {
    Properties {
        ...
        [HideInInspector]_node_5360 ("node_5360", Color) = (0.7132353,0.7132353,0.7132353,1)
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                diffuseColor = DiffuseAndSpecularFromMetallic( diffuseColor, (_Specular_var.rgb*_Specularity).r, specularColor, specularMonochrome );
                ...
                float3 indirectSpecular = (gi.indirect.specular + (saturate(( saturate((0.5 - 2.0*(_MainTexture_var.rgb-0.5)*(_node_5360.rgb-0.5))) > 0.5 ? (1.0-(1.0-2.0*(saturate((0.5 - 2.0*(_MainTexture_var.rgb-0.5)*(_node_5360.rgb-0.5)))-0.5))*(1.0-_Specular_var.rgb)) : (2.0*saturate((0.5 - 2.0*(_MainTexture_var.rgb-0.5)*(_node_5360.rgb-0.5)))*_Specular_var.rgb) ))*_Specularity)) * specularAO;
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float3 specColor;
                float4 _Specular_var = tex2D(_Specular,TRANSFORM_TEX(i.uv0, _Specular));
                diffColor = DiffuseAndSpecularFromMetallic( diffColor, (_Specular_var.rgb*_Specularity).r, specColor, specularMonochrome );
                float roughness = 1.0;
