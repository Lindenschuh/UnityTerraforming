...
Shader "NMP/Rocks" {
    Properties {
        ...
        [MaterialToggle] _Gray ("Gray", Float ) = 0.2149824
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 node_8188 = saturate(((_Color.rgb*_MainTexture_var.rgb)+_Shadows-1.0));
                float3 diffuseColor = lerp( node_8188, dot(node_8188,float3(0.3,0.59,0.11)), _Gray ); 
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 _MainTexture_var = tex2D(_MainTexture,TRANSFORM_TEX(i.uv0, _MainTexture));
                float3 node_8188 = saturate(((_Color.rgb*_MainTexture_var.rgb)+_Shadows-1.0));
                float3 diffColor = lerp( node_8188, dot(node_8188,float3(0.3,0.59,0.11)), _Gray );
                float specularMonochrome;
                float3 specColor;
                float4 _Specular_var = tex2D(_Specular,TRANSFORM_TEX(i.uv0, _Specular));
