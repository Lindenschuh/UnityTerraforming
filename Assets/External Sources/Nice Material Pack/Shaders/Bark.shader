...
Shader "NMP/Bark" {
    Properties {
        ...
        _Response ("Response", Range(0, 1)) = 1
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 node_2786_skew = i.uv0 + 0.2127+i.uv0.x*0.3713*i.uv0.y;
                float2 node_2786_rnd = 4.789*sin(489.123*(node_2786_skew));
                float node_2786 = frac(node_2786_rnd.x*node_2786_rnd.y*(1+node_2786_skew.x));
                float4 _Heightmap_var = tex2D(_Heightmap,TRANSFORM_TEX(i.uv0, _Heightmap));
                float3 node_4884 = (_Specular_var.rgb*_Response*node_2786*_Heightmap_var.rgb);
                float LdotH = max(0.0,dot(lightDirection, halfDirection));
                float4 _MainTexture_var = tex2D(_MainTexture,TRANSFORM_TEX(i.uv0, _MainTexture));
                float3 diffuseColor = saturate(((_Color.rgb*_MainTexture_var.rgb)+_Shadows-1.0)); 
                float specularMonochrome;
                float3 specularColor;
                diffuseColor = DiffuseAndSpecularFromMetallic( diffuseColor, (_Specular_var.rgb*0.193).r, specularColor, specularMonochrome );
                ...
                float3 indirectSpecular = (gi.indirect.specular + node_4884) * specularAO;
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float3 specColor;
                float4 _Specular_var = tex2D(_Specular,TRANSFORM_TEX(i.uv0, _Specular));
                diffColor = DiffuseAndSpecularFromMetallic( diffColor, (_Specular_var.rgb*0.193).r, specColor, specularMonochrome );
                float roughness = 1.0;
